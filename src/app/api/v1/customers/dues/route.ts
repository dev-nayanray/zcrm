...
export async function GET(request: NextRequest) {
  try {
    ...
    const minDue = sp.get("minDue") ? Number(sp.get("minDue")) : undefined;
    const from = sp.get("from") ? new Date(sp.get("from")!) : undefined;
    const to = sp.get("to") ? new Date(sp.get("to")!) : undefined;
    const res = await CustomerDueService.list({ page: q.page, limit: q.limit, search: q.search, minDue, status, from, to });
